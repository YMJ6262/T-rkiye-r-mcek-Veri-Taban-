...
11. **Map mixed dozens of languages/scripts.** The standard OSM tile layer renders every place name in that region's own local script — 北京市, موريتانيا, Magyarország — so a single world view showed labels in whatever language each area's OSM data happens to use. → Switched to Wikimedia's `osm-intl` tile layer (`?lang=en`), which forces a single consistent English label language, requires no API key, and looks visually identical to the familiar OSM style. Also enabled `detectRetina` so high-DPI screens get sharp @2x tiles while standard screens don't pay the extra bandwidth for them.

> Note: like the standard OSM tile server, Wikimedia's tile service is a free shared resource intended for moderate use. If this site ever gets heavy production traffic, consider a paid provider (e.g. MapTiler, Stadia Maps) with an API key instead.
