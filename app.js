...
function initMap() {
  ...
  // Basemap: Esri's "Dark Gray Canvas" (base + reference/labels), a muted background
  // basemap meant to sit behind data markers — a natural fit for this UI's dark theme,
  // and free with no API key. It's Esri's own authored world reference data rather than
  // raw OSM tiles, so at the world/country zoom levels this app actually uses it labels
  // places in English instead of each region's local script. (Caveat, disclosed rather
  // than hidden: Esri's docs note that a few densely-mapped regions — mainly Europe at
  // street-level zoom — can show a local-language label alongside the English one. This
  // app never zooms in that far, so it isn't something you'll actually see here.)
  const esriAttribution = 'Tiles &copy; Esri &mdash; Esri, HERE, Garmin, FAO, NOAA, USGS, &copy; OpenStreetMap contributors';
  ...
  // Safety net: the Wikimedia layer this project used before broke without warning when
  // Wikimedia locked it to their own sites. So this no longer just picks one provider and
  // hopes — if the primary basemap can't load its tiles, the map falls back automatically
  // to the standard OpenStreetMap layer instead of showing a blank grid of broken images.
  let tileErrorCount = 0;
  let fellBack = false;
  function fallBackToStandardOSM() {
    if (fellBack) return;
    fellBack = true;
    console.warn('Primary basemap failed to load — falling back to standard OpenStreetMap tiles.');
  ...
  baseLayer.on('tileerror', () => {
    tileErrorCount++;
    if (tileErrorCount > 3) fallBackToStandardOSM();
  });
